...
균열 표적은 `firstContactTarget`과 `noReward`를 가진 공격 불능 gunner입니다. `triggerPhaseRift()`가 이 표적에서 발동할 때만 `claimFirstContactReward()`가 실행됩니다. 보상은 런당 한 번만 청구되며, 기존 `pendingLevelUps`가 없을 때 부족한 XP만 `gainXP()`로 지급합니다. 이미 레벨업이 예약된 상태라면 두 번째 예약을 만들지 않고 첫 접촉 보상 청구만 기록합니다.

## 튜토리얼 입력 중립화

`InputManager.beginNeutralGate()`는 튜토리얼 단계 전환 시 현재 키보드, 포인터, 터치, 게임패드 입력이 모두 중립이 된 뒤 0.15초가 지나야 해제됩니다. 게이트가 켜진 동안 `getMove()`, `getAim()`, `isFiring()`, `consumeDash()`, `consumeEcho()`는 모두 중립값을 반환하고 `updateEchoTutorial()`은 진행률을 0으로 고정합니다.

`scripts/verify-7.1-first-contact.mjs`는 브라우저에서 신선 저장 첫 접촉, 게이트 중 웨이브 정지, 보상 중복 방지, cleanup 경로, 키보드/포인터/터치/합성 게임패드 중립화를 검증합니다.
...
현재 `CACHE_NAME`은 `echo-rift-first-contact-v7.1.0`입니다. fetch 전략은 요청 종류별로 분리했습니다.
