# ECHO RIFT: OVERTURE 7.1 — 기술 노트 (FIRST CONTACT)

## 7.1 목표

FIRST CONTACT는 첫 일반 런의 초반 체인을 `안전한 실제 적 처치 → 전투용 3초 기록 → 잔향 호출 → 같은 적 협공 → 첫 위상 균열 → 첫 강화 선택`으로 고정합니다. 콘텐츠와 밸런스 수치를 추가하지 않고, 기존 필드 코치에 첫 접촉 컨텍스트와 페이싱 게이트를 붙였습니다.

## 첫 접촉 상태 경계

`runOnboarding`은 런 단위 로컬 상태입니다. `window.echoRiftStatus.onboarding`은 다음 값을 읽기 전용으로 노출합니다.

- `firstCombatTargetSpawnAt`, `firstKillAt`
- `firstEchoActivationAt`, `firstPhaseRiftAt`
- `firstUpgradeOpenedAt`, `firstUpgradeChosenAt`
- `fieldCoachCompletedAt`, `pacingGateReleasedAt`
- `maxActiveEnemiesBeforeFirstUpgrade`
- `firstContactRewardClaimed`, `rewardClaims`, `reservedLevelUps`

이 값은 온라인 텔레메트리로 전송되지 않으며 저장 내보내기 스키마에도 추가하지 않았습니다.

## 페이싱 게이트

첫 접촉 중 `firstContactGateActive()`가 true이면 `updateWave()`는 `currentWave.elapsed`, `currentWave.threat`, `spawnTimer`, `spawnRemaining`을 진행하지 않습니다. `gameTime`, 플레이어 기록, 잔향, 탄환, 충돌, 코치 진행은 계속 흐릅니다. 첫 강화 선택 뒤 `releaseFirstContactGate('upgrade-selected')`가 게이트를 내리고 일반 웨이브 `spawnTimer`를 0.6~0.9초로 재설정합니다.

타임아웃, 수동 숨김, 훈련 진입, 죽음, 승리, 메뉴 복귀, QA 웨이브 변경, 월드 초기화는 모두 `cleanupFirstContactOnboarding()`을 통해 임시 표적을 `noReward` 사망 처리하고 게이트를 해제합니다.

## 임시 표적과 보상

워밍업 표적은 `firstContactWarmup` 플래그를 가진 공격 불능 wisp입니다. 실제 처치/점수/소량 XP는 사용하지만 `summoned`라 웨이브 클리어 조건에는 들어가지 않습니다.

균열 표적은 `firstContactTarget`과 `noReward`를 가진 공격 불능 gunner입니다. `triggerPhaseRift()`가 이 표적에서 발동할 때만 `claimFirstContactReward()`가 실행됩니다. 보상은 런당 한 번만 청구되며, 기존 `pendingLevelUps`가 없을 때 `player.xpGain`을 역산해 부족한 XP만 `gainXP()`로 지급합니다. 이미 레벨업이 예약된 상태라면 두 번째 예약을 만들지 않고 첫 접촉 보상 청구만 기록합니다.

## 튜토리얼 입력 중립화

`InputManager.beginNeutralGate()`는 튜토리얼 단계 전환 시 현재 키보드, 포인터, 터치, 게임패드 입력이 모두 중립이 된 뒤 0.15초가 지나야 해제됩니다. 게이트가 켜진 동안 `getMove()`, `getAim()`, `isFiring()`, `consumeDash()`, `consumeEcho()`는 모두 중립값을 반환하고 `updateEchoTutorial()`은 진행률을 0으로 고정합니다. 게이트 활성 중 새로 들어온 pointer/touch down은 `blockedPointers`에 기록되고, 실제 pointerup/pointercancel 전에는 중립 타이머가 진행되지 않습니다.

`scripts/verify-7.1-first-contact.mjs`는 브라우저에서 신선 저장 첫 접촉, 게이트 중 웨이브 정지, 보상 중복 방지, `player.xpGain > 1`, 기존 `pendingLevelUps > 0`, cleanup 경로, 키보드/포인터/터치/합성 게임패드 중립화를 검증합니다.
...
- 강화 카드 본문 클릭은 선택으로 이어지고, 잠금 버튼 클릭은 선택으로 이어지지 않는지 확인
...
현재 `CACHE_NAME`은 `echo-rift-first-contact-v7.1.1`입니다. fetch 전략은 요청 종류별로 분리했습니다.
